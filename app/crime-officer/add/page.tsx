...
const SOCO_LABS_OPTIONS = ANNEX_01_SOCO_LABS.map((s) => ({
  value: s,
  label: s,
}));
const RANK_OPTIONS = ANNEX_12_RANK.map((s) => ({ value: s, label: s }));
const SPOUSE_DESIGNATION_OPTIONS = ANNEX_07_SPOUSE_DESIGNATION.map((s) => ({
  value: s,
  label: s,
}));
...
function defaultForm(): FormData {
  return {
    ...
    alGeneralKnowledge: "",
    degreesBefore: [
      { id: newId(), degree: "", university: "", yearFrom: "", yearTo: "" },
    ],
    degreesAfter: [
      { id: newId(), degree: "", university: "", yearFrom: "", yearTo: "" },
    ],
  ...
  return (
    <div className="flex items-start gap-3 mb-6 pb-3 border-b border-slate-200">
      ...
      <div className="pt-0.5">
        ...
        {titleSi && (
          <p className="text-xs text-gray-500 mt-1 font-noto-sinhala">
            {titleSi}
          </p>
        )}
      </div>
...
function FieldLabel({ label, si }: { label: string; si?: string }) {
  return (
    <label className="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide leading-tight">
      {label}
      {si && (
        <span className="text-gray-400 normal-case tracking-normal ml-1 font-noto-sinhala">
          / {si}
        </span>
      )}
    </label>
  ...
  return (
    <div className="flex gap-3">
      {(["Yes", "No"] as const).map((opt) => {
        ...
        const base =
          "min-h-10 flex items-center gap-1.5 cursor-pointer text-sm px-3 py-2 rounded-lg border transition-colors";
...
export default function AddOfficerPage() {
  ...
  const updateChild = (id: number, patch: Partial<ChildRow>) =>
    set(
      "children",
      form.children.map((c) => (c.id === id ? { ...c, ...patch } : c)),
    );
  ...
  const updatePromotion = (id: number, patch: Partial<PromotionRow>) =>
    set(
      "promotions",
      form.promotions.map((p) => (p.id === id ? { ...p, ...patch } : p)),
    );
  const removePromotion = (id: number) =>
    set(
      "promotions",
      form.promotions.filter((p) => p.id !== id),
    );
  ...
  const updateBeforeCourse = (
    ...
    set(
      section,
      form[section].map((row) => (row.id === id ? { ...row, ...patch } : row)),
    );
  };

  const addBeforeCourse = (
    section: "localBeforeCourses" | "foreignBeforeCourses",
  ) => {
    setForm((f) => ({
      ...f,
  ...
  const updateAssignmentDraft = (
    section: "transfer" | "specialDuty",
    patch: Partial<AssignmentRow>,
  ) => {
    setForm((f) => {
      const currentDraft =
        section === "transfer" ? f.transferDraft : f.specialDutyDraft;
      const nextDraft = { ...currentDraft, ...patch };

      if (patch.from !== undefined || patch.to !== undefined) {
        nextDraft.duration = formatAssignmentDuration(
          nextDraft.from,
          nextDraft.to,
        );
      }
  ...
  const addAssignmentRecord = (section: "transfer" | "specialDuty") => {
    setForm((f) => {
      const draft =
        section === "transfer" ? f.transferDraft : f.specialDutyDraft;
      const duration =
        draft.duration || formatAssignmentDuration(draft.from, draft.to);
      const reasonValid =
        draft.reason === "Other"
          ? Boolean(draft.reasonOther.trim())
          : Boolean(draft.reason);
      const isComplete = Boolean(
        draft.socoLab && draft.from && draft.to && reasonValid && duration,
      );
  ...
  const removeAssignmentRecord = (
    ...
    setForm((f) =>
      section === "transfer"
        ? {
            ...f,
            transferHistory: f.transferHistory.filter((row) => row.id !== id),
          }
        : {
            ...f,
            specialDutyHistory: f.specialDutyHistory.filter(
              (row) => row.id !== id,
            ),
          },
    );
  ...
  const updateOLMandatory = (index: number, grade: OLGrade) => {
    const updated = form.olMandatorySubjects.map((s, i) =>
      i === index ? { ...s, grade } : s,
    );
  ...
  const updateALSubject = (id: number, patch: Partial<ALSubjectRow>) => {
    set(
      "alSubjects",
      form.alSubjects.map((s) => (s.id === id ? { ...s, ...patch } : s)),
    );
  };
  ...
  const removeALSubject = (id: number) => {
    ...
    set(
      "alSubjects",
      form.alSubjects.filter((s) => s.id !== id),
    );
  };

  const updateDegree = (
    section: "degreesBefore" | "degreesAfter",
    id: number,
    patch: Partial<DegreeRow>,
  ) => {
    set(
      section,
      form[section].map((d) => (d.id === id ? { ...d, ...patch } : d)),
    );
  };
  ...
  const canConfirmTransfer = Boolean(
    ...
    (form.transferDraft.reason !== "Other" ||
      form.transferDraft.reasonOther.trim()),
  );
  ...
  const canConfirmSpecialDuty = Boolean(
    ...
    (form.specialDutyDraft.reason !== "Other" ||
      form.specialDutyDraft.reasonOther.trim()),
  );
  ...
  return (
    <PageLayout>
      ...
      <form
        ...
        className="bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden animate-fade-in"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
              ...
              <div className="flex-1 min-w-0">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <DatePicker
                      value={form.dateJoinedSoco}
                      onChange={(v) => set("dateJoinedSoco", v)}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <GInput
                      value={form.socoCourseNo}
                      onChange={(v) => set("socoCourseNo", v)}
                  ...
                  <div>
                    ...
                    <GInput
                      value={form.socoService}
                      onChange={(v) => set("socoService", v)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:col-span-2 xl:col-span-3">
                    ...
                    <div>
                      ...
                      <GInput
                        value={form.telMobile}
                        onChange={(v) => set("telMobile", v)}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide font-noto-sinhala">
                          Name of the Child / දරුවාගේ නම
                        </th>
                        <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-48 font-noto-sinhala">
                          Birthday / උපන්දිනය
                        </th>
                        <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide font-noto-sinhala">
                          Status / තත්වය
                        ...
                        >
                          ...
                          <td className="px-2 py-1.5">
                            <DatePicker
                              ...
                              onChange={(v) =>
                                updateChild(child.id, { birthday: v })
                              }
                            />
                          </td>
                          <td className="px-2 py-1.5">
          ...
          <div className="p-4 sm:p-5 rounded-xl border border-indigo-200 bg-indigo-50/65">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              <div>
                ...
                <DatePicker
                  value={form.dateJoinedPolice}
                  onChange={(v) => set("dateJoinedPolice", v)}
                />
              </div>
              <div>
                ...
                <CustomSelect
                  ...
                  onChange={(v) => set("appointedRank", v)}
                  options={RANK_OPTIONS}
                  placeholder="Select"
                />
              </div>
                      ...
                      >
                        <td className="px-2 py-1.5">
                          <CustomSelect
                            value={promo.rank}
                            onChange={(v) =>
                              updatePromotion(promo.id, { rank: v })
                            }
                            options={RANK_OPTIONS}
                            placeholder="Rank"
                          />
                        ...
                        <td className="px-2 py-1.5">
                          <DatePicker
                            value={promo.date}
                            onChange={(v) =>
                              updatePromotion(promo.id, { date: v })
                            }
                          />
                        </td>
                        <td className="px-2 py-1.5 align-middle text-right whitespace-nowrap w-px">
          ...
          <div className="p-4 sm:p-5 rounded-xl border border-violet-200 bg-violet-50/60">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-violet-50 border-b border-violet-100">
                      ...
                      <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-56">
                        Grade
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {form.olMandatorySubjects.map((row, idx) => (
                      <tr
                        key={row.subject}
                        className="border-b border-violet-50 last:border-0 odd:bg-white even:bg-violet-50/20"
                      >
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {OL_GRADES.map((g) => (
                              <ToggleChip
                                ...
                                size="grade"
                                active={row.grade === g}
                                activeVariant={g === "F" ? "danger" : "violet"}
                  ...
                  <tbody>
                    {form.olOptionalSubjects.map((row, idx) => (
                      <tr
                        ...
                        className="border-b border-violet-50 last:border-0 odd:bg-white even:bg-violet-50/20"
                      >
                        <td className="px-3 py-2 text-xs text-gray-400 font-semibold">
                          {idx + 1}
                        </td>
                        <td className="px-2 py-1.5">
                          <GInput
                            value={row.subject}
                            onChange={(v) =>
                              updateOLOptional(row.id, { subject: v })
                            }
                          ...
                          <div className="flex gap-1.5 flex-wrap">
                            {OL_GRADES.map((g) => (
                              <ToggleChip
                                ...
                                activeVariant={g === "F" ? "danger" : "violet"}
                                onClick={() =>
                                  updateOLOptional(row.id, { grade: g })
                                }
                              >
                                {g}
                              </ToggleChip>
                ...
                <div className="rounded-xl border border-violet-100 overflow-hidden">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                          ...
                          <th className="text-left px-3 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide w-32">
                            Grade
                          </th>
                          <th className="px-2 py-2.5 w-px whitespace-nowrap">
                            <span className="sr-only">Actions</span>
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {form.alSubjects.map((row, idx) => (
                          <tr
                            ...
                            className="border-b border-gray-100 last:border-0"
                          >
                            <td className="px-3 py-1.5 text-xs text-gray-400 font-semibold">
                              {idx + 1}
                            </td>
                            <td className="px-2 py-1.5">
                              <GInput
                                value={row.subject}
                                onChange={(v) =>
                                  updateALSubject(row.id, { subject: v })
                                }
                                placeholder="Subject name"
                              />
                            </td>
                            <td className="px-2 py-1.5">
                              <GInput
                                value={row.grade}
                                onChange={(v) =>
                                  updateALSubject(row.id, { grade: v })
                                }
                  ...
                  {form.alSubjects.length < 5 && (
                    <div className="px-4 pb-3">
                      <AddRowButton onClick={addALSubject}>
                        Add Subject
                      </AddRowButton>
                    </div>
                  )}
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              <div className="rounded-xl border border-violet-100 bg-white overflow-hidden shadow-sm">
                <div className="px-4 py-3 border-b border-violet-100 bg-violet-50/60">
                  <h4 className="text-sm font-bold text-violet-900 uppercase tracking-wide">
                    Degrees / Qualifications Before Joining Police
                  </h4>
                  <p className="text-xs text-violet-700 mt-0.5">
                    Obtained prior to joining the Police Department
                  </p>
                </div>
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        {form.degreesBefore.length > 1 && (
                          <RemoveRowButton
                            onClick={() =>
                              removeDegree("degreesBefore", row.id)
                            }
                            size="sm"
                          />
                        )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div>
                          ...
                          <GInput
                            value={row.degree}
                            onChange={(v) =>
                              updateDegree("degreesBefore", row.id, {
                                degree: v,
                              })
                            }
                ...
                {form.degreesBefore.length < 6 && (
                  <div className="px-4 pb-4">
                    <AddRowButton onClick={() => addDegree("degreesBefore")}>
                      Add Qualification
                    </AddRowButton>
                  </div>
                )}
              ...
              <div className="rounded-xl border border-violet-100 bg-white overflow-hidden shadow-sm">
                <div className="px-4 py-3 border-b border-violet-100 bg-violet-50/60">
                  <h4 className="text-sm font-bold text-violet-900 uppercase tracking-wide">
                    Degrees / Qualifications After Joining Police (Sponsored)
                  </h4>
                  <p className="text-xs text-violet-700 mt-0.5">
                    Sponsored degrees obtained after joining the Police
                    Department
                  </p>
                </div>
                <div className="p-4 space-y-3">
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        {form.degreesAfter.length > 1 && (
                          <RemoveRowButton
                            onClick={() => removeDegree("degreesAfter", row.id)}
                            size="sm"
                          />
                        )}
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        <div>
                          ...
                          <GInput
                            value={row.degree}
                            onChange={(v) =>
                              updateDegree("degreesAfter", row.id, {
                                degree: v,
                              })
                            }
                            placeholder="e.g. LLB"
                ...
                {form.degreesAfter.length < 6 && (
                  <div className="px-4 pb-4">
                    <AddRowButton onClick={() => addDegree("degreesAfter")}>
                      Add Qualification
                    </AddRowButton>
                  </div>
                )}
          ...
          <div className="p-5 sm:p-6 rounded-2xl border border-amber-200 bg-amber-50/70">
            ...
            <p className="text-sm text-amber-900/80 mb-4">
              Capture departmental and external training completed before
              joining the SOCO project.
            </p>
            ...
            <div className="grid grid-cols-1 gap-5">
              <div className="rounded-xl border border-amber-100 bg-white shadow-sm overflow-hidden">
                  ...
                  <table className="min-w-[760px] w-full text-sm">
                    ...
                    <tbody>
                      {form.localBeforeCourses.map((row) => (
                        <tr
                          key={row.id}
                          className="border-b border-gray-100 odd:bg-white even:bg-amber-50/20 last:border-0"
                        >
                          <td className="px-2 py-1.5">
                            <GInput
                              value={row.conNo}
                ...
                <div className="px-4 pb-3">
                  <AddRowButton
                    onClick={() => addBeforeCourse("localBeforeCourses")}
                  >
                    Add row
                  </AddRowButton>
                </div>
              ...
              <div className="rounded-xl border border-amber-100 bg-white shadow-sm overflow-hidden">
                ...
                <div className="px-4 pb-3">
                  <AddRowButton
                    onClick={() => addBeforeCourse("foreignBeforeCourses")}
                  >
                    Add row
                  </AddRowButton>
                </div>
          ...
          <div className="p-5 sm:p-6 rounded-2xl border border-cyan-200 bg-cyan-50/70">
            ...
            <p className="text-sm text-cyan-900/80 mb-4">
              Record advanced courses and certifications completed after joining
              the SOCO project.
            </p>
            ...
            <div className="grid grid-cols-1 gap-5">
              <div className="rounded-xl border border-cyan-100 bg-white shadow-sm overflow-hidden">
                  ...
                  <table className="min-w-[760px] w-full text-sm">
                    ...
                    <tbody>
                      {form.localAfterCourses.map((row) => (
                        <tr
                          key={row.id}
                          className="border-b border-gray-100 odd:bg-white even:bg-cyan-50/20 last:border-0"
                        >
                          <td className="px-2 py-1.5">
                            <GInput
                              value={row.courseName}
                              onChange={(v) =>
                                updateAfterCourse("localAfterCourses", row.id, {
                                  courseName: v,
                                })
                              }
                            />
                          ...
                          <td className="px-2 py-1.5 align-middle text-right whitespace-nowrap w-px">
                            {form.localAfterCourses.length > 1 ? (
                              <RemoveRowButton
                                onClick={() =>
                                  removeAfterCourse("localAfterCourses", row.id)
                                }
                              />
                            ) : null}
                          </td>
                ...
                <div className="px-4 pb-3">
                  <AddRowButton
                    onClick={() => addAfterCourse("localAfterCourses")}
                  >
                    Add row
                  </AddRowButton>
                </div>
              ...
              <div className="rounded-xl border border-cyan-100 bg-white shadow-sm overflow-hidden">
                ...
                <div className="px-4 pb-3">
                  <AddRowButton
                    onClick={() => addAfterCourse("foreignAfterCourses")}
                  >
                    Add row
                  </AddRowButton>
                </div>
          ...
          <div className="p-5 sm:p-6 rounded-2xl border border-rose-200 bg-rose-50/70">
            <SectionHeader sectionNo={7} title="Driving License Details" titleSi="රියදුරු බලපත්‍ර විස්තර" />

            <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
              <div className="rounded-xl border border-rose-100 bg-white shadow-sm p-4 xl:col-span-2">
                <h4 className="text-sm font-bold text-rose-900 uppercase tracking-wide mb-3">
                  License Information
                </h4>
                <div className="space-y-4">
                  <div>
                    <FieldLabel label="Driving License No / රියදුරු බලපත්‍ර අංකය" />
                    <GInput
                  ...
                  <div>
                    <FieldLabel label="Categories of Vehicles / වාහන මාදිලිය" />
                    ...
                    >
                      {ANNEX_13_CATEGORY_OPTIONS.map((opt) => {
                        const selected = form.vehicleCategories.includes(
                          opt.value,
                        );
                        return (
                          <label
              ...
              <div className="rounded-xl border border-rose-100 bg-white shadow-sm p-4 xl:col-span-1">
                <h4 className="text-sm font-bold text-rose-900 uppercase tracking-wide">
                  Police Driving / Riding Qualified
                </h4>
                <p className="text-xs text-rose-700 mt-1 mb-4">
                  Select only Yes or No for each vehicle type.
                </p>
                ...
                <div className="space-y-3">
                  <div className="rounded-lg border border-gray-200 bg-gray-50/70 px-3 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <span className="text-sm font-semibold text-gray-800">
                      Heavy Vehicle / බර වාහන{" "}
                    </span>
                    <YesNo
                      value={form.heavyVehicleQualified}
                      onChange={(v) => set("heavyVehicleQualified", v)}
                    />
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-gray-50/70 px-3 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <span className="text-sm font-semibold text-gray-800">
                      Light Vehicle / සැහැල්ලු වාහන{" "}
                    </span>
                    <YesNo
                      value={form.lightVehicleQualified}
                      onChange={(v) => set("lightVehicleQualified", v)}
                    />
                  </div>
                  <div className="rounded-lg border border-gray-200 bg-gray-50/70 px-3 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <span className="text-sm font-semibold text-gray-800">
                      Motor Cycle / යතුරු පැදී
                    </span>
                    <YesNo
                      value={form.motorcycleQualified}
                      onChange={(v) => set("motorcycleQualified", v)}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-fuchsia-200 bg-fuchsia-50/80 p-4 sm:p-5 shadow-sm">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <div className="lg:col-span-3">
                  <FieldLabel label="SOCO LAB" si="SOCO සේවා ස්ථානය" />
                  <CustomSelect
                    value={form.transferDraft.socoLab}
                    onChange={(v) =>
                      updateAssignmentDraft("transfer", { socoLab: v })
                    }
                    options={SOCO_LABS_OPTIONS}
                    placeholder="Select SOCO LAB"
                  />
                ...
                <div className="lg:col-span-3">
                  ...
                  <DatePicker
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("transfer", { from: v })
                    }
                  />
                </div>
                <div className="lg:col-span-3">
                  ...
                  <DatePicker
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("transfer", { to: v })
                    }
                  />
                </div>
                <div className="lg:col-span-3">
                ...
                <div className="lg:col-span-3">
                  ...
                  <GInput
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("transfer", { oic: v })
                    }
                ...
                <div className="lg:col-span-6 space-y-2">
                  <div>
                    ...
                    <CustomSelect
                      ...
                      onChange={(v) =>
                        updateAssignmentDraft("transfer", {
                          reason: v,
                          reasonOther: "",
                        })
                      }
                ...
                <div className="lg:col-span-3 flex items-end">
                  <AddRowButton onClick={() => addAssignmentRecord("transfer")}>
                    Add transfer record
                  </AddRowButton>
                </div>
              </div>
                ...
                <tbody>
                  {form.transferHistory.length > 0 ? (
                    form.transferHistory.map((row) => (
                      <tr
                        key={row.id}
                        className="border-b border-fuchsia-50 last:border-0"
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <RemoveRowButton
                            onClick={() =>
                              removeAssignmentRecord("transfer", row.id)
                            }
                          >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <div className="lg:col-span-3">
                  ...
                  <CustomSelect
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("specialDuty", { socoLab: v })
                    }
                    options={SOCO_LABS_OPTIONS}
                    placeholder="Select SOCO LAB"
                  />
                ...
                <div className="lg:col-span-3">
                  ...
                  <DatePicker
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("specialDuty", { from: v })
                    }
                  />
                </div>
                <div className="lg:col-span-3">
                  ...
                  <DatePicker
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("specialDuty", { to: v })
                    }
                  />
                </div>
                <div className="lg:col-span-3">
                ...
                <div className="lg:col-span-3">
                  ...
                  <GInput
                    ...
                    onChange={(v) =>
                      updateAssignmentDraft("specialDuty", { oic: v })
                    }
                    placeholder="Officer in charge"
                  />
                </div>
                ...
                <div className="lg:col-span-6 space-y-2">
                  <div>
                    ...
                    <CustomSelect
                      ...
                      onChange={(v) =>
                        updateAssignmentDraft("specialDuty", {
                          reason: v,
                          reasonOther: "",
                        })
                      }
                    ...
                    form.specialDutyHistory.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-right">
                          <RemoveRowButton
                            onClick={() =>
                              removeAssignmentRecord("specialDuty", row.id)
                            }
                          >
                            <Trash2 className="h-3.5 w-3.5" />
                            Delete
          ...
          <div className="p-5 sm:p-6 rounded-2xl border border-emerald-200 bg-emerald-50/70">
            <SectionHeader
              ...
              titleSi="විනය විමර්ශන"
            />
            <p className="text-sm text-emerald-900/80 mb-4">
              Record current inquiry status and any relevant findings for each
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
              <div className="rounded-xl border border-emerald-100 bg-white shadow-sm p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
                  ...
                  <YesNo
                    value={form.orderlyRoomStatus}
                    onChange={(v) =>
                      set("orderlyRoomStatus", v as ToggleChoice)
                    }
                  />
                </div>
                <div>
              ...
              <div className="rounded-xl border border-emerald-100 bg-white shadow-sm p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
                  ...
                  <YesNo
                    value={form.preliminaryInquiryStatus}
                    onChange={(v) =>
                      set("preliminaryInquiryStatus", v as ToggleChoice)
                    }
                  />
                </div>
                <div>
              ...
              <div className="rounded-xl border border-emerald-100 bg-white shadow-sm p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
                  ...
                  <YesNo
                    value={form.disciplinaryInquiryStatus}
                    onChange={(v) =>
                      set("disciplinaryInquiryStatus", v as ToggleChoice)
                    }
                  />
                </div>
                <div>
          ...
          <div className="p-5 sm:p-6 rounded-2xl border border-sky-200 bg-sky-50/70">
            ...
            <p className="text-sm text-sky-900/80 mb-4">
              Record any known medical conditions and additional notes relevant
              to this officer.
            </p>
